...
export function MyPageSavedDraftsSection() {
  ...
  return (
    <section className="mt-8 reels-glass-card rounded-2xl p-5 sm:p-6">
      <h2 className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-zinc-500">
        임시 저장한 편집
      </h2>
      ...
      {cards.length === 0 ? (
        <p className="mt-4 text-[13px] text-zinc-500">아직 임시 저장한 항목이 없어요.</p>
      ) : (
        <ul className="mt-4 space-y-3">
          {cards.map(({ videoId, savedAt, video }) => {
            const owned = hasPurchased(video.id);
            return (
              <li key={`${videoId}-${savedAt}`} className="flex items-center gap-3 rounded-xl border border-white/10 bg-black/25 p-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={video.poster} alt="" className="h-14 w-14 rounded-md object-cover" />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[13px] font-bold text-zinc-100">{video.title}</p>
