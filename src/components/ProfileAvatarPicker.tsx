  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
      ...
      <div className="min-w-0 w-full flex-1">
        <p className="text-center text-[13px] font-bold text-zinc-100 [html[data-theme='light']_&]:text-zinc-900 sm:text-left">
          프로필 이미지
        </p>
        {hint ? (
        ...
        <div
          className={`mt-4 w-full rounded-xl border border-reels-cyan/20 bg-gradient-to-br from-black/40 to-black/20 p-2.5 sm:p-3 [html[data-theme='light']_&]:border-zinc-200 [html[data-theme='light']_&]:from-zinc-100/90 [html[data-theme='light']_&]:to-white ${
        ...
        >
          ...
          {customizerDisabled ? (
            ...
            <div className="mt-2 grid grid-cols-2 gap-1 sm:gap-x-2 sm:gap-y-1">
              <CycleRow
                label="헤어"
                current={parts.hair}
...
function CycleRow<T extends string>({
  ...
  return (
    <div className="flex min-w-0 items-center gap-0.5 rounded-lg border border-white/10 bg-black/25 px-1 py-0.5 sm:gap-1 sm:px-1.5 [html[data-theme='light']_&]:border-zinc-200 [html[data-theme='light']_&]:bg-white/80">
      <button
        type="button"
        onClick={() => onChange(cycleInList(list, current, -1))}
        className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md border border-white/10 text-zinc-300 transition hover:border-reels-cyan/40 hover:text-reels-cyan sm:h-7 sm:w-7 [html[data-theme='light']_&]:border-zinc-200 [html[data-theme='light']_&]:text-zinc-700"
        aria-label={`${label} 이전`}
      >
...
function GenderHeaderToggle({
  ...
  return (
    <div
      className="inline-flex max-w-full overflow-hidden rounded-lg border border-white/10 bg-black/25 p-0.5 [html[data-theme='light']_&]:border-zinc-200 [html[data-theme='light']_&]:bg-white/80"
      role="group"
      aria-label="성별"
    >
      {opts.map((o) => (
        <button
          ...
          className={`min-w-[2.75rem] rounded-md px-2 py-1 text-[10px] font-bold transition disabled:cursor-not-allowed disabled:opacity-40 sm:min-w-[3.25rem] ${
            value === o.id
