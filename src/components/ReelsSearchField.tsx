...
export function ReelsSearchField({
  ...
  if (exploreWatchExpand) {
    return (
      <form
        className="group relative ml-auto flex h-9 max-w-[2.75rem] shrink-0 flex-row items-stretch overflow-hidden rounded-full border border-white/15 bg-white/[0.06] shadow-none backdrop-blur-0 transition-[max-width] duration-300 ease-[cubic-bezier(0.33,1,0.45,1)] hover:max-w-[min(15rem,38vw)] focus-within:max-w-[min(15rem,38vw)] motion-reduce:max-w-[min(15rem,38vw)] motion-reduce:transition-none sm:hover:max-w-[min(17rem,34vw)] sm:focus-within:max-w-[min(17rem,34vw)] [html[data-theme='dark']_&]:border-white/20 [html[data-theme='dark']_&]:bg-white/[0.1] [html[data-theme='light']_&]:border-zinc-200 [html[data-theme='light']_&]:bg-zinc-50"
        onSubmit={(e) => {
          e.preventDefault();
          runSearch();
        }}
      >
        <input
          ...
          autoComplete="off"
          enterKeyHint="search"
          className={`mall-search min-h-0 min-w-0 flex-1 border-0 bg-transparent py-0 pl-3 pr-1 text-[13px] text-zinc-100 outline-none ring-0 transition-[opacity,color] duration-200 ease-out focus:ring-0 ${placeholderClass} rounded-none [html[data-theme='dark']_&]:text-zinc-100 [html[data-theme='light']_&]:text-zinc-900`}
          aria-label="릴스·키워드 검색"
        />
        {showTrailingIcon ? (
          <button
            type="submit"
            className={`${pinkSubmitTileClass} my-auto mr-1`}
            aria-label="검색 실행"
          >
