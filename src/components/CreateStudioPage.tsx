...
export function CreateStudioPage() {
  ...
  if (!videoId) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        <p className="text-[15px] font-semibold text-zinc-200">선택된 동영상이 없어요</p>
  ...
  if (!video) {
    return (
      <div className="mx-auto max-w-lg px-4 py-20 text-center">
        <p className="text-[15px] font-semibold text-zinc-200">해당 동영상을 찾을 수 없어요</p>
  ...
  //         동영상 상세에서 「모션 권한 구매(데모)」를 완료하면 「AI 창작하기」가 활성화됩니다.
  //       </p>
  //       <Link
  //         href={`/video/${video.id}`}
  //         className="mt-8 inline-flex rounded-full bg-reels-crimson px-6 py-3 text-[14px] font-extrabold text-white shadow-reels-crimson hover:brightness-110"
