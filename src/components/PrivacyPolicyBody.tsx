...
import { PRIVACY_ARTICLES_EN } from "@/lib/i18n/privacyPolicyEn";
import type { PrivacyBlock } from "@/lib/i18n/privacyPolicyKo";
import { PRIVACY_ARTICLES_KO } from "@/lib/i18n/privacyPolicyKo";
...
const subCls =
  "mt-4 font-semibold text-zinc-100 [html[data-theme='light']_&]:text-zinc-800";

function BlockView({ block }: { block: PrivacyBlock }) {
  if (block.type === "p") {
    return <p className={pCls}>{block.text}</p>;
  }
  if (block.type === "sub") {
    return <p className={subCls}>{block.text}</p>;
  }
  return (
    <ul className={ulCls}>
...
export function PrivacyPolicyBody() {
  ...
  const articles = loc === "en" ? PRIVACY_ARTICLES_EN : PRIVACY_ARTICLES_KO;
