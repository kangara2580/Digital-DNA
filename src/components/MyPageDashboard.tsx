...
export function MyPageDashboard() {
  ...
  return (
    <main className="mx-auto min-h-[60vh] max-w-[1500px] px-4 py-10 text-zinc-100 sm:px-6 sm:py-12 lg:px-8">
      <h1 className="text-2xl font-extrabold tracking-tight sm:text-3xl">마이페이지</h1>
      <p className="mt-2 text-[14px] text-zinc-400">기본정보를 먼저 확인하고, 왼쪽 메뉴에서 세부 항목을 관리하세요.</p>
              ...
              return (
                <Link
                  ...
                  className={`block rounded-xl border px-3 py-2.5 transition ${
                    active
                      ? "border-reels-cyan/45 bg-reels-cyan/15 text-zinc-100"
            ...
            <div className="reels-glass-card rounded-2xl p-5 sm:p-6">
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl border border-white/10 bg-black/25 p-4">
                  <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-500">아이디</p>
                  <p className="mt-2 text-[16px] font-extrabold text-zinc-100">{userId}</p>
                  <p className="mt-1 text-[12px] text-zinc-500">닉네임/이메일 연동 전 기본 계정입니다.</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-black/25 p-4">
                  <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-500">계정 상태</p>
                  <p className="mt-2 text-[16px] font-extrabold text-zinc-100">정상</p>
                  <p className="mt-1 text-[12px] text-zinc-500">홈 · 카테고리 · 창작 스튜디오 이용 가능</p>
                </div>
              </div>

              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl border border-white/10 bg-black/25 p-4">
                  <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-500">임시 저장</p>
                  <p className="mt-1 text-[22px] font-extrabold text-zinc-100">{draftCount}</p>
                  <p className="text-[11px] text-zinc-500">저장된 편집 항목</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-black/25 p-4">
                  <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-500">프로필 얼굴</p>
                  <p className="mt-1 text-[15px] font-extrabold text-zinc-100">{profileLabel}</p>
                  <p className="text-[11px] text-zinc-500">창작 시 적용되는 얼굴 정보</p>
                </div>
                <div className="rounded-xl border border-white/10 bg-black/25 p-4">
                  <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-zinc-500">바로가기</p>
              ...
              <ul className="mt-4 flex flex-wrap gap-4">
                {DEMO_FACE_PROFILES.map((p) => (
                  <li key={p.id} className="text-center">
                    ...
                    <img src={p.src} alt="" className="mx-auto h-20 w-20 rounded-full border border-white/12 object-cover" />
